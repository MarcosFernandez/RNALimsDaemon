...
class LimsRnaSeq():
    ...
    def run(self,directory=None):
        ...
        laneGtfStats = glob.glob(directory + "/*.filtered.gtf.stats.json")
        laneGeneCounts = glob.glob(directory + "/*.filtered.gtf.counts.txt")
        rmDupsFiles = glob.glob(directory + "/*.rmdup.lane.flagstat")

        rmDupsLibrary = {}

        with open(directory + "/duplicates.json") as json_file:
            rmDupsLibrary = json.load(json_file)

        configuration = {}
        with open(directory + "/.config.gt") as json_file:
            configuration = json.load(json_file)

        if len(laneGtfStats) != len(laneGeneCounts) or len(laneGtfStats) != len(rmDupsFiles) or len(laneGtfStats) == 0:
            return """The number of gtf stats files is not the same as gene counts files or remove duplicates files. Please check \
        ...
        #2.1 Process each set of files [[gtfStats][geneCounts][flagStats]]
            if self.updateLaneLibraryNames(os.path.basename(gtfJsonFile)):
                self.delStats()
                self.queryDataBase()
                self.checkRnaSeqAssociatedToMappingObject()
                self.parseRawData(jsonFile = gtfJsonFile, geneCounts = geneCounts, flagstatsFile = rmDupFile, configuration=configuration)
                self.updateLaneToLIMS()
            else:
                return "Sorry!! File %s does not comply with the pattern SAMPLE_LIBRARY_FLOWCELL_LANE at the begining of the name. \
                      This pattern must be respected to get sample, library and flowcell information"


        if len(rmDupsLibrary) < 1:
            return "No remove duplicates files per library are found. Please check *.rmdup.library.flagstat files."

        #3 For each flagstat file
        for libStats in sorted(rmDupsLibrary.keys()) :
            if self.updateSampleLibraryNames(libStats):
                self.delStats()   
                self.checkRnaSeqAssociatedToLibraryObject()
                self.checkRnaSeqMappingAssociatedToLibraryObject()
                self.parseRawDataLibrary(libStats,rmDupsLibrary)
                self.updateLibraryToLIMS()
            else:
                return "Sorry!! File %s does not comply with the pattern SAMPLE_LIBRARY at the begining of the name. \
